...
def build_gold_news_prompt(price_snapshot: PriceSnapshot) -> str:
    """Build the prompt sent to the Claude Agent SDK for `/analyze/gold/news`.

    Unlike `build_gold_prompt()`, this asks only for a curated list of recent
    news items (no recommendation/technical analysis) - the price is embedded
    purely as context for relevance, not as something to reason about.
    """
    formatted_price = format_rupiah(price_snapshot.price)
    change_desc = _describe_change(price_snapshot.change_pct)

    context = (
        "Anda adalah asisten riset yang mengumpulkan berita terkini seputar emas untuk "
        "investor ritel emas fisik di Indonesia.\n\n"
        "KONTEKS HARGA (hanya sebagai acuan relevansi, jangan dicari ulang):\n"
        f"- Harga emas saat ini: {formatted_price} per gram\n"
        f"- Perubahan 24 jam: {change_desc}\n\n"
        "TUGAS ANDA:\n"
        "Gunakan web search untuk menemukan 5-8 berita TERKINI (beberapa hari terakhir) yang "
        "relevan dengan pergerakan harga emas, mencakup topik seperti harga emas global "
        "(XAU/USD), nilai tukar Rupiah, kebijakan bank sentral, geopolitik, aktivitas bank "
        "sentral membeli emas, dan faktor musiman/domestik Indonesia.\n\n"
        "ATURAN PENTING:\n"
        "- Tulis seluruh isi jawaban dalam Bahasa Indonesia.\n"
        "- Jawaban HARUS berupa satu objek JSON valid saja, tanpa teks lain di luar JSON, "
        "tanpa markdown code fence.\n\n"
    ...
    schema = (
        ...
        '  "items": [\n'
        "    {\n"
        '      "title": "judul berita, singkat dan spesifik",\n'
        '      "summary": "ringkasan 1-2 kalimat mengapa berita ini relevan dengan harga '
        'emas",\n'
        '      "source": "nama sumber berita, mis. Reuters, Bloomberg, Antara (boleh null '
        'bila tidak diketahui)",\n'
        '      "published_at": "tanggal atau perkiraan waktu publikasi, mis. \\"2026-01-05\\" '
        'atau \\"2 hari lalu\\" (boleh null bila tidak diketahui)"\n'
        "    }\n"
        "  ]\n"
        "}"
    )
